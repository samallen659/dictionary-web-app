...
function DropDown({ handleFontChange }: AppProps) {
    ...
    return (
        <div>
            ...
            {/* Dropdown menu */}
            <div id="dropdown" className="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 shadow-gray-200 shadow-lg dark:shadow-purple">
                <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                    {fontOptions.map((option, i) => (
                        <li key={i} className={`font-${fontOptions[i]} block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white ${selected === i && 'text-purple font-bold'}`} onClick={handleClick} value={i}>
                            {option}
                        </li>
                    ))}
                </ul>
